...
use ast_grep_core::{source::SgNode, AstGrep, Doc, Language};
...
// Instruction:
// create a `test.js`,
// run `cargo run -p ast-grep-oxc -- test.ts`
...
fn main() -> std::io::Result<()> {
  ...
  let doc = OxcDoc::try_new(source_text.clone(), lang).expect("Failed to parse");
  let sg = AstGrep::doc(doc);
  let pattern = Pattern::new("console.log($A)", lang);
  let node = sg.root();
...
impl OxcDoc {
  fn try_new(source: String, lang: OxcLang) -> Result<Self, String> {
