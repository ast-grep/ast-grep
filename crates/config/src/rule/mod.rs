...
";
  ...
  #[test]
  fn test_parameterized_matches_lowering() {
    let utils = from_str(
      r"
...
",
  ...
  #[test]
  fn test_multiple_parameterized_matches_lowering() {
    let utils = from_str(
      r"
first(ARG):
  matches: ARG
...
",
    ...
    match rule {
      Rule::All(all) => {
        assert_eq!(all.inner().len(), 2);
        assert!(all
          .inner()
          .iter()
          .all(|rule| matches!(rule, Rule::Matches(_))));
      }
